...
db-migrate: require-MSG ## Create a new DB migration, set MSG="<message>"
	...
	$(eval $@_DB_HOST := $(shell docker inspect --format '{{ .NetworkSettings.IPAddress }}' $(container_postgres_dev)))
	docker run --rm -v $(shell pwd)/db/migrations/:/migrations/ --entrypoint /bin/bash $(image_api_server_dev) -c 'migrate -database postgres://$(db_user_dev):$(db_pass_dev)@$($@_DB_HOST):5432/$(db_name_dev) create -ext sql -dir /migrations "$($@_MSG)"'
	sudo chown -R $(shell whoami):$(shell whoami) $(shell pwd)/db/migrations/

db-upgrade: ## Apply the most recent created DB migration
	$(eval $@_DB_HOST := $(shell docker inspect --format '{{ .NetworkSettings.IPAddress }}' $(container_postgres_dev)))
	docker run --rm -v $(shell pwd)/db/migrations/:/migrations/ --entrypoint /bin/bash $(image_api_server_dev) -c 'migrate -path=/migrations/ -database postgres://$(db_user_dev):$(db_pass_dev)@$($@_DB_HOST):5432/$(db_name_dev)?sslmode=disable up'

db-downgrade: ## Revert the most recent applied DB migration
	$(eval $@_DB_HOST := $(shell docker inspect --format '{{ .NetworkSettings.IPAddress }}' $(container_postgres_dev)))
	docker run --rm -v $(shell pwd)/db/migrations/:/migrations/ --entrypoint /bin/bash $(image_api_server_dev) -c 'migrate -path=/migrations/ -database postgres://$(db_user_dev):$(db_pass_dev)@$($@_DB_HOST):5432/$(db_name_dev)?sslmode=disable down 1'
